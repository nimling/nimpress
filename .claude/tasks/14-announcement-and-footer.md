...
2. nimpress: `footer` in the config becomes an object `{ text, copyright, social, navigation, generator }`; a string value is refused by the config schema with a message naming `footer.text`, and every place that reads the string reads `text`. `copyright` is one line rendered beside the social links. `social` is an array of `{ icon, link, name }` with the icon in the three forms `sidebar.icon` accepts, and `name` as the link title. `navigation: true` renders previous and next page links in sidebar order above the footer line. `generator: false` removes the `Built with nimpress` line that renders when unset.
