...
nimpress is a Svelte 5 static site generator designed to simplify building and maintaining project documentation. A site needs one config file and the `nimpress` cli, which owns Vite, so there is no build to wire by hand. Install it, [create your site](/get-started/create-your-site), and [publish it](/get-started/publish-your-site).
