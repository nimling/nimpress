...
export async function run(argv: string[]): Promise<void> {
  ...
  if (cmd === 'export') {
    runExport(cwd, resolved.contentDir, argv.slice(1))
    return
  }
...
function rewriteExportedPage(text: string, version: string): string {
...
function runExport(cwd: string, contentDir: string, args: string[]): void {
  const target = guardFlag(args, 'target') ?? ''
  const out = join(cwd, guardFlag(args, 'out') ?? '.nimpress-export')
  const contentRoot = join(cwd, contentDir)
  let version = ''
  try {
